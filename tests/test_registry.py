"""CRUD genérico, permisos, serialización y resolución de FK (sin Vertex)."""
import datetime
import decimal
...
class RegistryTests(TestCase):
    ...
    def test_crud_happy_path(self):
        pk = registry.create(self.staff, "auth.group", {"name": "Editores"})["pk"]
        self.assertEqual(registry.get(self.staff, "auth.group", pk)["name"], "Editores")
        self.assertEqual(len(registry.query(self.staff, "auth.group", filters={"name": "Editores"})), 1)
        registry.update(self.staff, "auth.group", pk, {"name": "Editores2"})
        self.assertEqual(registry.get(self.staff, "auth.group", pk)["name"], "Editores2")
        registry.delete(self.staff, "auth.group", pk)
        self.assertEqual(registry.query(self.staff, "auth.group", filters={"name": "Editores2"}), [])
