(function () {
  ...
  function build() {
    ...
    btn.className = "ag-btn"; btn.title = "Assistant";
    if (root.dataset.logo) {
      var logo = document.createElement("img");
      logo.src = root.dataset.logo; logo.alt = "Assistant"; logo.className = "ag-logo";
    ...
    panel.innerHTML =
      '<div class="ag-head"><span>Assistant</span><span class="ag-head-btns">' +
      '<span class="ag-reset" title="New conversation">↺</span><span class="ag-x" title="Close">✕</span></span></div>' +
      '<div class="ag-msgs"></div>' +
      '<div class="ag-foot"><textarea placeholder="Type a message…"></textarea><button class="ag-send">➤</button></div>';
  ...
  function typing() {
    var d = document.createElement("div");
    d.className = "ag-typing"; d.textContent = "thinking…";
    el.msgs.appendChild(d); scroll();
    return d;
  }
  ...
  function onSend() {
    ...
    post(URLS.message, { text: text, page_context: pageContext() })
      ...
      .catch(function () { t.remove(); bubble("ag-model", "Connection error."); })
      .finally(function () { setBusy(false); });
  }

  var ACTIONS = {
    create: { cls: "create", label: "Create", verb: "Create", icon: "＋" },
    update: { cls: "update", label: "Update", verb: "Save", icon: "✎" },
    delete: { cls: "delete", label: "Delete", verb: "Delete", icon: "🗑", danger: true },
  };
  ...
  function cardBody(p) {
    ...
    if (p.op === "delete") {
      box.appendChild(kvRow(p.model_verbose || "record", span(fmtVal(p.current) , "ag-v")));
      return box;
    }
    var data = p.data || {};
    Object.keys(data).forEach(function (k) {
      ...
      if (p.op === "update" && p.current && k in p.current) {
        ...
        box.appendChild(kvRow(k, ov === nv ? span(nv + "  (unchanged)", "ag-same") : diffNode(ov, nv)));
      } else {
  ...
  function confirmCard(c) {
    var p = c.preview || {}, meta = ACTIONS[c.op] || { cls: "", label: c.op, verb: "Confirm", icon: "•" };
    ...
    if (meta.danger) card.appendChild(span("⚠ This action cannot be undone.", "ag-warn"));
    ...
    var no = document.createElement("button"); no.className = "ag-no"; no.textContent = "Cancel";
